...
│   ├── file_tools.py    # 受限目录中的 UTF-8 文本读取工具
│   └── registry.py      # 工具定义，以及自动生成的注册表、权限表和 Schema
├── data/                # Agent 可读取的受限资料目录
│   └── agent_basics.txt # 文件读取工具的示例资料
├── main.py             # 命令行入口
├── tests/
...
- `file_tools.py`：`read_file(text)`；将 `text` 视为 `data/` 中的相对文件名，只读取 UTF-8 文本。代码会先解析路径并确认它仍位于 `data/`，拒绝 `../` 等越界输入；文件不存在时返回“文件不存在。”。
...
目前项目已有三个自动文本工具、一个受限文件读取工具和一个模拟敏感工具；没有多轮对话记忆。同一轮多个工具会按顺序执行，不使用真正并行，单次请求最多执行三次。`main.py` 仍是单次输入入口，尚未做成可持续等待确认的命令行循环。后续可以继续扩展：
