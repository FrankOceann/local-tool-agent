...
- 支持有先后依赖的请求：一次最多连续调用三次工具，例如先转大写，再统计单词数。
...
每一轮只允许调用一个工具，但单次用户请求最多可连续执行三轮。达到三次工具调用上限后，Agent 会要求模型直接生成最终回答，避免无限循环和持续消耗 API 额度。
...
4. **Agent 循环**：每次工具执行后，模型可以继续决定是否调用下一项工具；最多连续三轮。
5. **可测试性**：用 Fake Client 模拟模型响应，避免测试依赖网络和真实 API Key。

## 当前限制与下一步

目前项目已有三个文本工具、没有多轮对话记忆；每一轮只支持一个工具调用，单次请求最多连续执行三轮。后续可以继续扩展：
