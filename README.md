...
- 支持资料关键词搜索：`search_files` 只搜索项目 `data/` 目录中直接包含的 `.txt` 文件，返回包含关键词的文件名；没有匹配时返回可读提示。需要全文时，Agent 会再调用 `read_file`。
...
│   └── agent_basics.txt # 文件读取与关键词搜索的示例资料
├── main.py             # 命令行入口
├── tests/
...
- `file_tools.py`：提供两个受限资料工具：`read_file(text)` 将 `text` 视为 `data/` 中的相对文件名，只读取 UTF-8 文本；`search_files(text)` 将 `text` 视为关键词，只搜索该目录直接包含的 `.txt` 文件。读取前会先解析路径并确认它仍位于 `data/`，拒绝 `../` 等越界输入；文件不存在或搜索无结果时返回可读提示。
...
目前项目已有三个自动文本工具、两个受限资料工具（读取与搜索）和一个模拟敏感工具；没有多轮对话记忆。同一轮多个工具会按顺序执行，不使用真正并行，单次请求最多执行三次。`main.py` 仍是单次输入入口，尚未做成可持续等待确认的命令行循环。后续可以继续扩展：
