...
模型每一轮可以请求一个或多个工具；Python 始终按模型给出的顺序执行。单次用户请求最多执行三次工具调用；达到上限后，Agent 会要求模型直接生成最终回答，避免无限循环和持续消耗 API 额度。
...
目前项目已有三个文本工具、没有多轮对话记忆；同一轮多个工具会按顺序执行，不使用真正并行，单次请求最多执行三次。后续可以继续扩展：
