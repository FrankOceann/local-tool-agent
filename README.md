...
- 支持资料关键词搜索：`search_files` 只搜索项目 `data/` 目录中直接包含的 `.txt` 文件，返回包含关键词的文件名和内容；没有匹配时返回可读提示。
...
模型会调用 `search_files`，Python 只在 `data/` 目录的 `.txt` 文件中搜索，然后把结果交还给模型总结。它适合资料很多、用户并不知道准确文件名的场景；需要全文时，模型还可以继续调用 `read_file`。
