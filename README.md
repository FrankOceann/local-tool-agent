...
- 支持资料关键词搜索：`search_files` 只搜索项目 `data/` 目录中直接包含的 `.txt` 文件，返回包含关键词的文件名；英文关键词不区分大小写；没有匹配时返回可读提示。需要全文时，Agent 会再调用 `read_file`。
...
- `file_tools.py`：提供两个受限资料工具：`read_file(text)` 将 `text` 视为 `data/` 中的相对文件名，只读取 UTF-8 文本；`search_files(text)` 将 `text` 视为关键词，只搜索该目录直接包含的 `.txt` 文件，英文关键词不区分大小写。读取前会先解析路径并确认它仍位于 `data/`，拒绝 `../` 等越界输入；文件不存在或搜索无结果时返回可读提示。
