...
- 支持 RAG Top-K 检索质量实验：评测脚本会在同一份索引和评测集上依次输出 Top-1、Top-2、Top-3 的来源与命中结果。真实实验显示 Top-1 与 Top-2 均为 `3/4`，Top-3 为 `4/4`；当前保留 Top-3。
...
├── main.py             # 命令行入口
├── scripts/
│   └── evaluate_rag.py  # 输出 Top-1、Top-2、Top-3 RAG 评测结果
├── tests/
...
│   ├── test_rag_evaluation.py   # RAG 固定评测集与来源命中测试
│   ├── test_evaluate_rag_script.py # RAG 评测脚本循环测试
...
目前项目已有三个自动文本工具、四个受限资料工具（单文件读取、批量读取、关键词搜索与向量检索）和一个模拟敏感工具；它已具备真正的最小 RAG：文档 Chunk、云端 Embedding、本地余弦相似度 Top-K 与来源引用。固定评测集的真实实验已验证：Top-1 为 `3/4`、Top-2 为 `3/4`、Top-3 为 `4/4`，因此当前保留 Top-3。索引只在当前进程内缓存；重启后会重新向量化资料，因此长资料会产生外部 API 调用成本。同一轮多个工具会按顺序执行，不使用真正并行，单次请求最多执行三次。`main.py` 仍是单次输入入口，尚未做成可持续等待确认的命令行循环。后续可以继续扩展：

- 增加天气、网页搜索、数据库查询等工具。
- 对互不依赖的工具实现真正并行执行。
- 加入聊天历史与长期记忆。
- 将模拟保存替换为受限目录内的真实写入，并增加审计日志和更严格的参数规则。
- 为“资料不足”场景设计相似度阈值或拒答策略，并在独立评测实验中验证。
